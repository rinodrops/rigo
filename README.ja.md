...
肯定形の選択的展開 `[extra]` を追加。ホストまたはグループ配下に
path/tag を列挙すると，それらだけがその端末群に展開され，Vaultの残りは
従来の `[include]` / `[exclude]` 規則のまま。他ホストでは `excluded`。
両方に当たるときは `[exclude]` が勝ち，allowlist モードとも独立。
両READMEに記載を追加した。
